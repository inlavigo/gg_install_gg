...
void main() {
  ...
  group('.vscode/launch.json', () {
    test('pathes in launch.json', () {
      ...
      // Ensure there is a configuration for executing bin/gg_install_gg.dart
      final configurations =
          parsedLaunchJson['configurations'] as List<dynamic>;

      final ggInstallGg = configurations.firstWhere(
        (dynamic configuration) =>
            configuration['name'].toString() == 'gg_install_gg.dart',
      );

      expect(
        ggInstallGg,
        isNotNull,
        reason: 'Item for executing bin/gg_install_gg.dart not found',
      );

      expect(ggInstallGg['name'], 'gg_install_gg.dart');
      expect(ggInstallGg['type'], 'dart');
      expect(ggInstallGg['request'], 'launch');
